...
<details>
<summary>View Alternative Format (PlantUML)</summary>
...
### Architecture Execution Walkthrough (POST /api/v1/orders/checkout)

This execution flow involves **4 primary components**:
* **Frontend Client** (`Client`): User web/mobile application initiating the checkout request.
* **OrderController (Controller)** (`Ctrl`): Receives incoming HTTP requests and validates the request payload.
* **PaymentGateway (Stripe API)** (`PayGW`): Third-party credit card processing gateway.
* **PostgreSQL (Database)** (`DB`): Persistence layer storing order records.

#### Detailed Execution Steps:
1. **Client** dispatches an HTTP POST payload containing the shopping cart to **OrderController**.
2. **OrderController** verifies authorization & cart items; if invalid, immediately returns HTTP 400 Bad Request.
3. **OrderController** invokes **PaymentGateway** to process the credit card transaction.
4. **OrderController** updates the order status to `PAID` in **PostgreSQL**.
5. **OrderController** returns a successful HTTP 201 Created response to the client.

#### Error Pathways & Exception Handling:
* **CartEmptyException (400):** Occurs when the checkout cart contains no active items.
* **PaymentDeclinedException (402):** Occurs when credit card authorization is declined by the payment gateway.
* **DatabaseTransactionException (500):** Occurs if the database connection drops during transaction commit.
